...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		if (charArray.length == 1 && isVowel(charArray[0])) {
			return 1;
		}
		int start = 0;
		int end = 0;
		while (start < len) {
			while (start < len && !isVowel(charArray[start])) {
				start++;
			}
			end = start;
			while (end < len && isVowel(charArray[end])) {
				end++;
			}
			if (end != start) {
				cnt++;
		...
		if (word.charAt(len-1) == 'e' && !isVowel(word.charAt(len-2)) && cnt != 1) {
			cnt--;
		}
	    return cnt;
	}
	...
	public double getFleschScore()
	{
		...
		double score = 206.835 - 1.015 * (words / sents) - 84.6 * (sylls / words);
	    return score;
	}
}
